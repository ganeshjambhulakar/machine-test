...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <button class="btn btn-small btn-info"> <a href="/"  class="text-white"> Dashboard </a></button>
        ...
        <div class="card-body">
            <h3 class="card-title mb-3 text-center">Edit Bucket</h3>
            <form  method="POST">
                @CSRF
                <div class="form-group">
                    <label for="exampleInputEmail1">Bucket Name</label>
                    <input type="text" name="name" class="form-control" id="name"  placeholder="Enter bucket name" value="<?= $bucket[0]['name'] ?>">
                 </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">Bucket Volume  (In inches)</label>
                    <input type="number" name="volume" class="form-control" id="volume"  step="0.01" placeholder="Enter bucket volume" value="<?= $bucket[0]['volume'] ?>">
                </div>
